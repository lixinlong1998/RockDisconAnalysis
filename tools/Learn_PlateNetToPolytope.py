...
'''
基于有限结构面的三维复杂块体切割研究
DOI：10.16285/j.rsm.2016.08.011
岩体的块体结构和结构面网络模型生成，是进行各种力学分析和场分析的基础。详细研究了有限结构面进行复杂块
体切割的过程，提出了相应的算法，并采用 C++语言编写了相应的程序。为描述块体切割后的复连通特性，在块体数据结构
中添加了有向壳的概念。结构面可以为简单的凸多边形，也可采用形态更加复杂的凹多边形。通过面-面求交线、交线环路
搜索形成有向环、有向环包含关系分析形成有向面、有向面拓扑搜索形成有向壳和有向壳包含关系分析形成块体等过程，将
有限结构面分别与各块体进行切割运算，形成进行块体切割的一般方法。在切割过程中将得到的有向环，有向面、有向壳和
块体分别进行拓扑有效性校核，满足要求后得到最终的块体和结构面网络模型。选取 4 个算例来验证该方法的可行性。计算
结果表明，该方法可以对复杂块体进行有效地切割，结构面可以选择包括凹形面在内的复杂多边形，方法具有普遍意义。
关 键 词：有限结构面；块体切割；复杂块体；结构面网络模型
'''
...
def generate_random_disk():
    center = np.random.rand(3) * 10
    normal = np.random.randn(3)
    normal /= np.linalg.norm(normal)
    return (np.append(normal, -np.dot(normal, center)), center, normal)
...
def find_feasible_point(A, b):
    ...
    A_aug = np.hstack((A, np.ones((A.shape[0], 1)) * -1))
    b_aug = b

    res = linprog(c, A_ub=A_aug, b_ub=b_aug, method='highs')

    if res.x is None or res.x[-1] <= 1e-6:  # 检查松弛变量是否为正
        print("无严格可行区域")
        return None
    return res.x[:-1]  # 返回原始坐标
