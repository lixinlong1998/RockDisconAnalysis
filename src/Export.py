...
def _ensure_rgb255(color_spec):
    """
    将各种颜色表达转换为 0-255 的 (r,g,b) tuple，dtype=uint8。
    支持：
      - "#RRGGBB" / "RRGGBB"
      - "rgb(r,g,b)"
      - "r,g,b" 或 "r g b"
      - (r,g,b) / [r,g,b]，值可为 0-255 或 0-1 浮点
    """
    ...
    # tuple/list
    if isinstance(color_spec, (tuple, list)) and len(color_spec) == 3:
        a, b, c = color_spec
        # 0-1 浮点转 0-255
        if (isinstance(a, float) or isinstance(b, float) or isinstance(c, float)) and \
                (0.0 <= a <= 1.0 and 0.0 <= b <= 1.0 and 0.0 <= c <= 1.0):
            a, b, c = a * 255, b * 255, c * 255
        return (np.uint8(_clamp255(a)), np.uint8(_clamp255(b)), np.uint8(_clamp255(c)))
    ...
    if isinstance(color_spec, str):
        ...
        # rgb(r,g,b)
        m = re.fullmatch(r'(?i)rgb\(\s*([0-9.]+)\s*,\s*([0-9.]+)\s*,\s*([0-9.]+)\s*\)', s)
        if m:
            vals = [float(m.group(i)) for i in range(1, 4)]
            # 判断是不是 0-1 浮点
            if all(0.0 <= v <= 1.0 for v in vals):
                vals = [v * 255.0 for v in vals]
            return tuple(np.uint8(_clamp255(v)) for v in vals)

        # "r,g,b" 或 "r g b"
        parts = re.split(r'[,\s]+', s)
        if len(parts) == 3 and all(re.fullmatch(r'[0-9.]+', p) for p in parts):
    ...
    # 兜底灰
    return (np.uint8(128), np.uint8(128), np.uint8(128))


def export_all_ellipdisks(
        save_path,
        discontinuitys,
        cluster_attr='joint_id',
        resolution=60
):
    """
    按 cluster 给每个椭圆盘统一着色并导出为 PLY。
    - cluster_colors: List[str]，可多种颜色表达；若为 None 则全部灰色。
    - cluster_attr:   从 disc 上取 cluster id 的属性名（默认 'joint_cluster_id'）。
    - resolution:     每个盘边界采样点个数。
    """
    cluster_colors = ["#868686", "#E78686", "#E7BA86", "#DCE786", "#90E786", "#86E7D9", "#0096d9", "#869BE7"]  # 传入颜色字符串列表: ["#e41a1c", "0,255,0", "rgb(0,0,255)", ...]
    starttime = time.perf_counter()
    vertices_all, faces_all, colors_all = [], [], []
    vertex_offset = 0
    PI2 = 2 * np.pi

    # 颜色缓存 & cluster->index 映射
    color_cache = {}
    cluster_to_color_index = {}
    colors_list = list(cluster_colors) if cluster_colors else None
    n_colors = len(colors_list) if colors_list else 0

    def _get_rgb_for_cluster(cluster_id):
        # 没给颜色：统一灰
        if not colors_list or n_colors == 0:
            return (np.uint8(128), np.uint8(128), np.uint8(128))

        # 若 cluster_id 是可用的 0..n-1，直接索引
        if isinstance(cluster_id, int) and 0 <= cluster_id < n_colors:
            key = colors_list[cluster_id]
            if key not in color_cache:
                color_cache[key] = _ensure_rgb255(key)
            return color_cache[key]

        # 否则按出现顺序分配一个颜色索引（对 n_colors 取模）
        if cluster_id not in cluster_to_color_index:
            next_idx = len(cluster_to_color_index) % n_colors
            cluster_to_color_index[cluster_id] = next_idx
        key = colors_list[cluster_to_color_index[cluster_id]]
        if key not in color_cache:
            color_cache[key] = _ensure_rgb255(key)
        return color_cache[key]

    count = 0
    for disc in getattr(discontinuitys, 'discontinuitys', []):
        if getattr(disc, 'valid', True) is False:
            continue

        center = np.asarray(disc.disc_center, dtype=float)  # (3,)
        a_norm = np.asarray(disc.long_axis_norm, dtype=float)  # (3,)
        b_norm = np.asarray(disc.short_axis_norm, dtype=float)  # (3,)
        a_len = float(disc.ellip_a)
        b_len = float(disc.ellip_b)

        # 向量化生成边界点（更快）
        angles = np.linspace(0.0, PI2, resolution, endpoint=False)  # (res,)
        cos_t = np.cos(angles)[:, None]  # (res,1)
        sin_t = np.sin(angles)[:, None]  # (res,1)
        boundary_pts = center[None, :] + a_len * cos_t * a_norm[None, :] + b_len * sin_t * b_norm[None, :]

        # 顶点：中心 + 边界
        disk_coords = np.vstack([center[None, :], boundary_pts])  # (res+1, 3)
        vertices_all.append(disk_coords)

        # 颜色：按 cluster 一盘同色
        cluster_id = getattr(disc, cluster_attr, None)
        rgb = _get_rgb_for_cluster(cluster_id)
        disk_colors = np.tile(np.array(rgb, dtype=np.uint8)[None, :], (disk_coords.shape[0], 1))
        colors_all.append(disk_colors)
        ...
        vertex_offset += disk_coords.shape[0]
        count += 1

    # 汇总并导出
    if vertices_all:
        vertices_all = np.vstack(vertices_all).astype(np.float32)  # (N,3)
        colors_all = np.vstack(colors_all).astype(np.uint8)  # (N,3)
    else:
        vertices_all = np.empty((0, 3), dtype=np.float32)
        colors_all = np.empty((0, 3), dtype=np.uint8)

    faces_all = np.array(faces_all, dtype=np.int32) if faces_all else None
    edges_all = None  # 保持与你的导出器接口一致

    export_to_meshlab_ply(
        save_path,
    ...
    print(
        f'[time cost]{Workflow.time_cost_hms(time.perf_counter() - starttime)} — {count} discons: ellipdisks have been exported.')
